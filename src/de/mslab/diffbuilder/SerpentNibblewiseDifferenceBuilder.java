 ...
 * 
 */
public class SerpentNibblewiseDifferenceBuilder extends AbstractDifferenceBuilder {
	...
	protected void setValueAtPosition(int position, boolean v) {
		value.setBit(position, v);
		value.setBit(position + 32, v);
		value.setBit(position + 64, v);
		value.setBit(position + 96, v);
	}
	
	protected DifferenceIterator storeNextValue() {
		return new SerpentNibblewiseDifferenceIterator(value, dimension, bitPositions);
	}
