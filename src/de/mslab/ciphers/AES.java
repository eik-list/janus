...
/**
 * Abstract base class for the AES, as designed 1998 as Rijndael by Daemen and Rijmen.  
 * 
 */
abstract public class AES extends AbstractRoundBasedBlockCipher {
	...
	public static final int[] INVERSE_SBOX = { // inverse s-box
	...
	public static final int[] RCON = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a};
	
	public static final int[] SBOX = { // forward s-box
	...
	// Xtime3[x] = x^Xtime2[x];
	public static final int[] X_TIMES_2 = {0x00, 0x02, 0x04, 0x06, 0x08, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14, 0x16,
	...
	/* protected static final int[] X_TIMES_2_SBOX = { // combined Xtimes2[Sbox[]]
	...
	public static final int[] X_TIMES_3 = {0x00, 0x03, 0x06, 0x05, 0x0c, 0x0f, 0x0a, 0x09, 0x18, 0x1b, 0x1e, 0x1d,
	...
	/* protected static final int[] X_TIMES_3_SBOX = { // combined Xtimes3[Sbox[]]
		}; */
	public static final int[] X_TIMES_9 = {0x00, 0x09, 0x12, 0x1b, 0x24, 0x2d, 0x36, 0x3f, 0x48, 0x41, 0x5a, 0x53,
	...
	public ByteArray computeExpandedKey(ByteArray keyPart, int round) {
		checkKeySize(keyPart);
		
		int indexOfFirstByte = AES.NUM_BYTES_IN_STATE * round; // 16 * 1 = 16
		int expandedKeySize = (numRounds + 1) * stateSize;
		int numColumnsPerKey = keySize / AES.NUM_ROWS_IN_STATE; // 24 / 4 = 6
		int lastColumnIndex = AES.NUM_COLUMNS_IN_STATE * (numRounds + 1); // 4 * (13) = 52
		
		ByteArray expandedKey = new ByteArray(expandedKeySize);
		
		if (indexOfFirstByte + keySize > expandedKeySize) {
			indexOfFirstByte = expandedKeySize - keySize;
		}
		
		int indexOfFirstColumn = indexOfFirstByte / AES.NUM_COLUMNS_IN_STATE; // 16 / 4 = 4
		int firstColumnAfter = indexOfFirstColumn + (keySize / NUM_COLUMNS_IN_STATE); // 4 + 24 / 4 = 10
		expandedKey.copyBytes(keyPart, 0, indexOfFirstByte);
		
		// compute forwards
		for (int columnIndex = firstColumnAfter; columnIndex < lastColumnIndex; columnIndex++) {
			createKeyColumnForwards(expandedKey, columnIndex, numColumnsPerKey);
		}
		
		// compute backwards
		for (int columnIndex = indexOfFirstColumn - 1; columnIndex >= 0; columnIndex--) {
			createKeyColumnBackwards(expandedKey, columnIndex, numColumnsPerKey);
		}
		
		return expandedKey.splice(0, expandedKeySize);
	}
	...
	public ByteArray expandKey(ByteArray key) {
		final int expectedKeySize = (numRounds + 1) * stateSize;
		secretKey = new ByteArray(expectedKeySize);
		secretKey.copyBytes(key);
		
		int numColumnsPerKey = keySize / NUM_ROWS_IN_STATE;
		int firstColumnIndex = numColumnsPerKey;
		int lastColumnIndex = NUM_COLUMNS_IN_STATE * (numRounds + 1);
		
		for (int columnIndex = firstColumnIndex; columnIndex < lastColumnIndex; columnIndex++) {
			createKeyColumnForwards(secretKey, columnIndex, numColumnsPerKey);
		}
	...
	public ByteArray getRoundKey(int round) {
		...
		if (round > 0) {
			keyOffset = round * NUM_BYTES_IN_STATE;
		}
		
		ByteArray roundKey = new ByteArray(NUM_BYTES_IN_STATE);
		roundKey.xor(secretKey, 0, NUM_BYTES_IN_STATE, keyOffset);
		return roundKey;
	}
	
	public int getFirstColumnIndexInKeyOfRound(int round) {
		return round * NUM_ROWS_IN_STATE;
	}
	...
	public void setExpandedKey(ByteArray expandedKey) {
		checkExpandedKeySize(expandedKey);
		this.secretKey = expandedKey.clone();
	}
	
	protected void checkExpandedKeySize(ByteArray key) {
		final int expectedKeySize = (numRounds + 1) * stateSize;
	...
	protected void checkKeySize(ByteArray key) {
		if (key.length() != keySize) {
			throw new InvalidKeySizeError(key.length());
		}
	}
	
	public abstract void createKeyColumnBackwards(ByteArray key, int columnIndex, int numColumnsPerKey);
	public abstract void createKeyColumnForwards(ByteArray key, int columnIndex, int numColumnsPerKey);
	
	protected ByteArray invertMixColumns(ByteArray state) {
		...
		newState.set(2, X_TIMES_D[state.get(0)] ^ X_TIMES_9[state.get(1)] ^ X_TIMES_E[state.get(2)] ^ X_TIMES_B[state.get(3)]);
		newState.set(3, X_TIMES_B[state.get(0)] ^ X_TIMES_D[state.get(1)] ^ X_TIMES_9[state.get(2)] ^ X_TIMES_E[state.get(3)]);
		
		newState.set(4, X_TIMES_E[state.get(4)] ^ X_TIMES_B[state.get(5)] ^ X_TIMES_D[state.get(6)] ^ X_TIMES_9[state.get(7)]);
		newState.set(5, X_TIMES_9[state.get(4)] ^ X_TIMES_E[state.get(5)] ^ X_TIMES_B[state.get(6)] ^ X_TIMES_D[state.get(7)]);
		newState.set(6, X_TIMES_D[state.get(4)] ^ X_TIMES_9[state.get(5)] ^ X_TIMES_E[state.get(6)] ^ X_TIMES_B[state.get(7)]);
		newState.set(7, X_TIMES_B[state.get(4)] ^ X_TIMES_D[state.get(5)] ^ X_TIMES_9[state.get(6)] ^ X_TIMES_E[state.get(7)]);
		
		newState.set(8, X_TIMES_E[state.get(8)] ^ X_TIMES_B[state.get(9)] ^ X_TIMES_D[state.get(10)] ^ X_TIMES_9[state.get(11)]);
		newState.set(9, X_TIMES_9[state.get(8)] ^ X_TIMES_E[state.get(9)] ^ X_TIMES_B[state.get(10)] ^ X_TIMES_D[state.get(11)]);
		newState.set(10, X_TIMES_D[state.get(8)] ^ X_TIMES_9[state.get(9)] ^ X_TIMES_E[state.get(10)] ^ X_TIMES_B[state.get(11)]);
		newState.set(11, X_TIMES_B[state.get(8)] ^ X_TIMES_D[state.get(9)] ^ X_TIMES_9[state.get(10)] ^ X_TIMES_E[state.get(11)]);
		
		newState.set(12, X_TIMES_E[state.get(12)] ^ X_TIMES_B[state.get(13)] ^ X_TIMES_D[state.get(14)] ^ X_TIMES_9[state.get(15)]);
		newState.set(13, X_TIMES_9[state.get(12)] ^ X_TIMES_E[state.get(13)] ^ X_TIMES_B[state.get(14)] ^ X_TIMES_D[state.get(15)]);
		newState.set(14, X_TIMES_D[state.get(12)] ^ X_TIMES_9[state.get(13)] ^ X_TIMES_E[state.get(14)] ^ X_TIMES_B[state.get(15)]);
		newState.set(15, X_TIMES_B[state.get(12)] ^ X_TIMES_D[state.get(13)] ^ X_TIMES_9[state.get(14)] ^ X_TIMES_E[state.get(15)]);
		
		return newState;
	}
	
	protected ByteArray invertSubBytes(ByteArray state) {
		for (int index = 0; index < NUM_BYTES_IN_STATE; index++) {
			state.set(index, INVERSE_SBOX[state.get(index)]);
		}
		return state;
	}
	
	protected ByteArray invertShiftRows(ByteArray state) {
		int value;
		
		// unshift row 1
		value = state.get(13);
		state.set(13, state.get(9));
		state.set(9, state.get(5));
		state.set(5, state.get(1));
		state.set(1, value);
		
		// unshift row 2
		value = state.get(2);
		state.set(2, state.get(10));
		state.set(10, value);
		value = state.get(6);
		state.set(6, state.get(14));
		state.set(14, value);
		
		// unshift row 3
		value = state.get(3);
		state.set(3, state.get(7));
		state.set(7, state.get(11));
		state.set(11, state.get(15));
		state.set(15, value);
		return state;
	}
	
	protected ByteArray mixColumns(ByteArray state) {
		ByteArray newState = new ByteArray(stateSize);
		
		newState.set(0, X_TIMES_2[state.get(0)] ^ X_TIMES_3[state.get(1)] ^ state.get(2) ^ state.get(3));
		newState.set(1, state.get(0) ^ X_TIMES_2[state.get(1)] ^ X_TIMES_3[state.get(2)] ^ state.get(3));
		newState.set(2, state.get(0) ^ state.get(1) ^ X_TIMES_2[state.get(2)] ^ X_TIMES_3[state.get(3)]);
		newState.set(3, X_TIMES_3[state.get(0)] ^ state.get(1) ^ state.get(2) ^ X_TIMES_2[state.get(3)]);
		
		newState.set(4, X_TIMES_2[state.get(4)] ^ X_TIMES_3[state.get(5)] ^ state.get(6) ^ state.get(7));
		newState.set(5, state.get(4) ^ X_TIMES_2[state.get(5)] ^ X_TIMES_3[state.get(6)] ^ state.get(7));
		newState.set(6, state.get(4) ^ state.get(5) ^ X_TIMES_2[state.get(6)] ^ X_TIMES_3[state.get(7)]);
		newState.set(7, X_TIMES_3[state.get(4)] ^ state.get(5) ^ state.get(6) ^ X_TIMES_2[state.get(7)]);
		
		newState.set(8, X_TIMES_2[state.get(8)] ^ X_TIMES_3[state.get(9)] ^ state.get(10) ^ state.get(11));
		newState.set(9, state.get(8) ^ X_TIMES_2[state.get(9)] ^ X_TIMES_3[state.get(10)] ^ state.get(11));
		newState.set(10, state.get(8) ^ state.get(9) ^ X_TIMES_2[state.get(10)] ^ X_TIMES_3[state.get(11)]);
		newState.set(11, X_TIMES_3[state.get(8)] ^ state.get(9) ^ state.get(10) ^ X_TIMES_2[state.get(11)]);
		
		newState.set(12, X_TIMES_2[state.get(12)] ^ X_TIMES_3[state.get(13)] ^ state.get(14) ^ state.get(15));
		newState.set(13, state.get(12) ^ X_TIMES_2[state.get(13)] ^ X_TIMES_3[state.get(14)] ^ state.get(15));
		newState.set(14, state.get(12) ^ state.get(13) ^ X_TIMES_2[state.get(14)] ^ X_TIMES_3[state.get(15)]);
		newState.set(15, X_TIMES_3[state.get(12)] ^ state.get(13) ^ state.get(14) ^ X_TIMES_2[state.get(15)]);
		
		state = newState;
		return state;
	}

	protected ByteArray shiftRows(ByteArray state) {
		int value;
		
		// shift row 1
		value = state.get(1);
		state.set(1, state.get(5));
		state.set(5, state.get(9));
		state.set(9, state.get(13));
		state.set(13, value);
		...
		// shift row 3
		value = state.get(15);
		state.set(15, state.get(11));
		state.set(11, state.get(7));
		state.set(7, state.get(3));
		state.set(3, value);
		return state;
	}
	
	protected ByteArray subBytes(ByteArray state) {
		for (int index = 0; index < NUM_BYTES_IN_STATE; index++) {
