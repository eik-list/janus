package de.mslab.ciphers;

import de.mslab.core.ByteArray;
import de.mslab.errors.InvalidKeySizeError;


abstract class KLEIN extends AbstractRoundBasedBlockCipher {
	
	public static final int NUM_BYTES_IN_64_BIT = 64 / Byte.SIZE;
	
	protected static final short[] SBOX = { 0x7,0x4,0xA,0x9,0x1,0xF,0xB,0x0,0xC,0x3,0x2,0x6,0x8,0xE,0xD,0x5 };
	private static final int[] X_TIMES_2 = {};
	private static final int[] X_TIMES_3 = {};
	private static final int[] X_TIMES_9 = {};
	private static final int[] X_TIMES_B = {};
	private static final int[] X_TIMES_D = {};
	private static final int[] X_TIMES_E = {};
	
	public boolean canInvertKeySchedule() {
		return true;
	}
	
	public ByteArray computeExpandedKey(ByteArray keyPart, int round) {
		checkKeySize(keyPart.length());
		secretKey = new ByteArray((numRounds + 1) * keySize);
		
		if (round <= numRounds) {
			secretKey = expandKeyForwards(secretKey, keyPart.clone(), round);
		}
		
		if (round > 0) {
			secretKey = expandKeyBackwards(secretKey, keyPart.clone(), round);
		}
		
		return secretKey;
	}
	
	public ByteArray computeKeyPart(ByteArray expandedKey, int round) {
		int from = (round - 1) * keySize;
		int numBytesToCopy = keySize;
		
		ByteArray result = new ByteArray(keySize);
		result.copyBytes(expandedKey, from, 0, numBytesToCopy);
		return result;
	}
	
	public ByteArray decryptRounds(ByteArray block, int fromRound, int toRound) {
		state = block.clone();
		
		if (toRound == numRounds) {
			state = addRoundKey(toRound + 1, state);
		}
		
		for (int round = toRound; round >= fromRound; round--) {
			state = invertMixNibbles(state);
			// logger.info("invert {0}", state);
			state = invertRotateNibbles(state);
			// logger.info("rotate {0}", state);
			state = subNibbles(state);
			// logger.info("subnib {0}", state);
			state = addRoundKey(round, state);
			// logger.info("addkey {0}", state);
		}
		
		return state;
	}
	
	public ByteArray encryptRounds(ByteArray block, int fromRound, int toRound) {
		state = block.clone();
		
		for (int round = fromRound; round <= toRound; round++) {
			state = addRoundKey(round, state);
			state = subNibbles(state);
			state = rotateNibbles(state);
			state = mixNibbles(state);
		}
		
		if (toRound == numRounds) {
			state = addRoundKey(toRound + 1, state);
		}
		
		return state;
	}
	
	public int getNumActiveComponentsInKeySchedule() {
		return 4 * numRounds;
	}
	
	public ByteArray getRoundKey(int round) {
		int from = (round - 1) * keySize;
		int to = from + stateSize;
		return this.secretKey.splice(from, to);
	}
	
	public boolean hasKeyInjectionInRound(int round) {
		return round >= 1 && round <= numRounds + 1;
	}
	
	public boolean injectsKeyAtRoundBegin(int round) {
		return true;
	}
	
	public boolean operatesNibblewise() {
		return true;
	}
	
	public void setKey(ByteArray key) {
		checkKeySize(key.length());
		this.secretKey = expandKey(key.clone());
	}
	
	public void setExpandedKey(ByteArray expandedKey) {
		checkExpandedKeySize(expandedKey.length());
		this.secretKey = expandedKey.clone();
	}
	
	private void checkKeySize(int length) {
		if (length != keySize) {
			throw new InvalidKeySizeError(length);
		}
	}
	
	private void checkExpandedKeySize(int length) {
		if (length != (numRounds + 1) * keySize) {
			throw new InvalidKeySizeError(length);
		}
	}
	
	private ByteArray expandKey(ByteArray key) {
		int numRoundKeys = numRounds + 1;
		secretKey = new ByteArray(numRoundKeys * keySize);
		secretKey = expandKeyForwards(secretKey, key, 1);
		return secretKey;
	}
	
	private ByteArray expandKeyBackwards(ByteArray expandedKey, ByteArray key, int round) {
		ByteArray roundKey = key;
		int halfLength = keySize / 2;
		
		// logger.info("key to expand {0}", key);
		expandedKey.copyBytes(roundKey, 0, (round - 1) * keySize);
		
		for (round--; round >= 1; round--) {
			// logger.info("roundkey     {0}", roundKey);
			subKey(roundKey, halfLength); // S-box is its own inverse
			// logger.info("after subkey {0}", roundKey);
			roundKey.set(2, roundKey.get(2) ^ (round));
			// logger.info("after xor    {0}", roundKey);
			invertFeistelKey(roundKey, halfLength);
			// logger.info("after feist  {0}", roundKey);
			
			invertRotateHalfKey(roundKey, 0, halfLength);
			invertRotateHalfKey(roundKey, halfLength, keySize);
			// logger.info("after rotate {0}", roundKey);
			expandedKey.copyBytes(roundKey, 0, (round - 1) * keySize);
		}
		
		return expandedKey;
	}
	
	private ByteArray expandKeyForwards(ByteArray expandedKey, ByteArray key, int round) {
		ByteArray roundKey = key;
		int halfLength = keySize / 2;
		
		expandedKey.copyBytes(key, 0, (round - 1) * keySize);
		
		for (round++; round <= numRounds + 1; round++) {
			rotateHalfKey(roundKey, 0, halfLength);
			rotateHalfKey(roundKey, halfLength, keySize);
			feistelKey(roundKey, halfLength);
			roundKey.set(2, roundKey.get(2) ^ (round - 1));
			subKey(roundKey, halfLength);
			
			expandedKey.copyBytes(roundKey, 0, (round - 1) * keySize);
		}
		
		return expandedKey;
	}
	
	private void subKey(ByteArray roundKey, int halfLength) {
		int value, msb, lsb;
		int start = halfLength + 1;
		int end = start + 2;
		
		for (int i = start; i < end; i++) {
			value = roundKey.get(i);
			msb = SBOX[(value >> 4) & 0xF] << 4; 
			lsb = SBOX[value & 0xF]; 
			roundKey.set(i, msb | lsb);
		}
	}
	
	private void feistelKey(ByteArray roundKey, int halfLength) {
		ByteArray left = roundKey.splice(0, halfLength);
		ByteArray right = roundKey.splice(halfLength);
		left.xor(right);
		roundKey.copyBytes(right);
		roundKey.copyBytes(left, 0, halfLength);
	}
	
	private void invertFeistelKey(ByteArray roundKey, int halfLength) {
		ByteArray left = roundKey.splice(0, halfLength);
		ByteArray right = roundKey.splice(halfLength);
		right.xor(left);
		roundKey.copyBytes(right);
		roundKey.copyBytes(left, 0, halfLength);
	}
	
	private void invertRotateHalfKey(ByteArray roundKey, int from, int to) {
		short temp = roundKey.get(to - 1);
		
		for (int i = to - 1; i > from; i--) {
			roundKey.set(i, roundKey.get(i - 1));
		}

		roundKey.set(from, temp);
	}
	
	private void rotateHalfKey(ByteArray roundKey, int from, int to) {
		short temp = roundKey.get(from);
		
		for (int i = from + 1; i < to; i++) {
			roundKey.set(i - 1, roundKey.get(i));
		}
		
		roundKey.set(to - 1, temp);
	}

	protected ByteArray invertMixNibbles(ByteArray state) {
		ByteArray newState = new ByteArray(state.length());
		newState.set(0, X_TIMES_E[state.get(0)] ^ X_TIMES_B[state.get(1)] ^ X_TIMES_D[state.get(2)] ^ X_TIMES_9[state.get(3)]);
		newState.set(1, X_TIMES_9[state.get(0)] ^ X_TIMES_E[state.get(1)] ^ X_TIMES_B[state.get(2)] ^ X_TIMES_D[state.get(3)]);
		newState.set(2, X_TIMES_D[state.get(0)] ^ X_TIMES_9[state.get(1)] ^ X_TIMES_E[state.get(2)] ^ X_TIMES_B[state.get(3)]);
		newState.set(3, X_TIMES_B[state.get(0)] ^ X_TIMES_D[state.get(1)] ^ X_TIMES_9[state.get(2)] ^ X_TIMES_E[state.get(3)]);
		
		newState.set(4, X_TIMES_E[state.get(4)] ^ X_TIMES_B[state.get(5)] ^ X_TIMES_D[state.get(6)] ^ X_TIMES_9[state.get(7)]);
		newState.set(5, X_TIMES_9[state.get(4)] ^ X_TIMES_E[state.get(5)] ^ X_TIMES_B[state.get(6)] ^ X_TIMES_D[state.get(7)]);
		newState.set(6, X_TIMES_D[state.get(4)] ^ X_TIMES_9[state.get(5)] ^ X_TIMES_E[state.get(6)] ^ X_TIMES_B[state.get(7)]);
		newState.set(7, X_TIMES_B[state.get(4)] ^ X_TIMES_D[state.get(5)] ^ X_TIMES_9[state.get(6)] ^ X_TIMES_E[state.get(7)]);
		return newState;
	}
	
	private ByteArray invertRotateNibbles(ByteArray state) {
		short first = state.get(stateSize - 2);
		short second = state.get(stateSize - 1);
		
		for (int i = stateSize - 1; i >= 2; i--) {
			state.set(i, state.get(i - 2));
		}
		
		state.set(0, first);
		state.set(1, second);
		return state;
	}
	
	protected ByteArray mixNibbles(ByteArray state) {
		ByteArray newState = new ByteArray(state.length());
		newState.set(0, X_TIMES_2[state.get(0)] ^ X_TIMES_3[state.get(1)] ^ state.get(2) ^ state.get(3));
		newState.set(1, X_TIMES_2[state.get(1)] ^ X_TIMES_3[state.get(2)] ^ state.get(3) ^ state.get(0));
		newState.set(2, X_TIMES_2[state.get(2)] ^ X_TIMES_3[state.get(3)] ^ state.get(0) ^ state.get(1));
		newState.set(3, X_TIMES_2[state.get(3)] ^ X_TIMES_3[state.get(0)] ^ state.get(1) ^ state.get(2));
		
		newState.set(4, X_TIMES_2[state.get(4)] ^ X_TIMES_3[state.get(5)] ^ state.get(6) ^ state.get(7));
		newState.set(5, X_TIMES_2[state.get(5)] ^ X_TIMES_3[state.get(6)] ^ state.get(4) ^ state.get(7));
		newState.set(6, X_TIMES_2[state.get(6)] ^ X_TIMES_3[state.get(7)] ^ state.get(4) ^ state.get(5));
		newState.set(7, X_TIMES_2[state.get(7)] ^ X_TIMES_3[state.get(4)] ^ state.get(5) ^ state.get(6));
		return newState;
	}
	
	private ByteArray rotateNibbles(ByteArray state) {
		short first = state.get(0);
		short second = state.get(1);
		
		for (int i = 2; i < stateSize; i++) {
			state.set(i - 2, state.get(i));
		}
		
		state.set(stateSize - 2, first);
		state.set(stateSize - 1, second);
		return state;
	}
	
	private ByteArray subNibbles(ByteArray state) {
		int value, msb, lsb;
		
		for (int i = 0; i < stateSize; i++) {
			value = state.get(i);
			msb = SBOX[(value >> 4) & 0xF] << 4; 
			lsb = SBOX[value & 0xF]; 
			state.set(i, msb | lsb);
		}
		
		return state;
	}
	
}
